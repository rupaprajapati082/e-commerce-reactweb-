...
const Checkout = () => {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/order/create`, 
    ...
    <div className="min-h-screen bg-[#FDFDFD]">
      ...
      <div className="bg-[#f9f9f9] py-10 px-6 md:px-20 border-b border-slate-100">
        ...
        <div className="flex items-center gap-2 mt-2 text-xs text-slate-400 uppercase font-bold tracking-widest">
          ...
          <span>/</span>
          <Link to="/cart" className="hover:text-[#89C74A]">Cart</Link>
          <span>/</span>
      ...
      <div className="px-6 md:px-20 py-16">
        <form onSubmit={handlePlaceOrder} className="grid grid-cols-1 lg:grid-cols-12 gap-16">
          ...
          <div className="lg:col-span-7 space-y-10">
            <div>
              <h2 className="text-lg font-bold text-[#222] uppercase tracking-widest mb-8 border-b border-slate-100 pb-4">Billing Details</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input 
                    required 
                    type="text"
                    className="w-full px-6 py-4 border border-slate-200 rounded-sm text-sm focus:outline-none focus:border-[#89C74A] bg-slate-50 transition-all"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-1">Town / City *</label>
                  <input 
                    required 
                    type="text"
                    className="w-full px-6 py-4 border border-slate-200 rounded-sm text-sm focus:outline-none focus:border-[#89C74A] bg-slate-50 transition-all"
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-1">Postcode / ZIP *</label>
                  <input 
                    required 
                    type="text"
                    className="w-full px-6 py-4 border border-slate-200 rounded-sm text-sm focus:outline-none focus:border-[#89C74A] bg-slate-50 transition-all"
            ...
            <div className="space-y-4">
              <h2 className="text-lg font-bold text-[#222] uppercase tracking-widest mb-4">Additional Information</h2>
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-1">Order Notes (Optional)</label>
                <textarea 
          ...
          <div className="lg:col-span-5">
            <div className="bg-white border border-slate-100 p-8 sticky top-32">
              <h2 className="text-lg font-bold text-[#222] uppercase tracking-widest mb-8 border-b border-slate-100 pb-4">Your Order</h2>
              
              <div className="space-y-4 mb-8">
                <div className="flex justify-between text-[10px] font-bold uppercase tracking-widest text-slate-400 pb-2 border-b border-slate-50">
                  <span>Product</span>
                  <span>Subtotal</span>
                </div>
                {cart?.items.map(item => (
                ...
                <div className="pt-4 border-t border-slate-100 space-y-4">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-[#89C74A] font-bold uppercase text-[10px]">Free Shipping</span>
                  </div>
              ...
              <div className="bg-[#f9f9f9] p-6 rounded-sm mb-8">
                <div className="flex items-center gap-3 mb-4">
                  <input type="radio" checked readOnly className="accent-[#89C74A]" />
              ...
              <button 
                type="submit" 
                className="block w-full bg-[#89C74A] text-white text-center py-4 rounded-full font-bold uppercase text-[11px] tracking-widest hover:bg-[#7ab33f] transition-all shadow-lg shadow-[#89C74A]/20"
              >
                Place Order
              </button>
            </div>
